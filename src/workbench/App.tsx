        ...
        <Panel defaultSize={54} minSize={36} order={2}>
          <section className="chat-canvas" aria-label="Conversation">
            ...
            <section className="conversation">
              ...
              <section data-testid="opl-workbench-delivery-mode" className="delivery-mode" aria-label="Delivery mode">
              ...
              >
                <Tabs.List aria-label="Artifact previews">
                  {model.artifactPreviews.map((preview) => (
                    <Tabs.Trigger key={preview.id} value={preview.id}>
                      {preview.label}
                    </Tabs.Trigger>
                  ))}
                </Tabs.List>
                {model.artifactPreviews.map((preview) => (
                  <Tabs.Content key={preview.id} value={preview.id} className="artifact-preview">
                    <header>
                      <h3>{preview.title}</h3>
            ...
            <Dialog.Root open modal={false}>
              <Dialog.Content data-testid="opl-provenance-drawer" className="provenance-drawer">
                ...
                <button
                  ...
                  type="button"
                  onClick={() => runDryRun("candidate.delivery.export", { refs: model.deliverables.map((item) => item.ref) })}
                >
